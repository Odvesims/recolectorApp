...
export default class BluetoothManagerComponent extends Component {
    ...
    componentDidMount() {
		if(!BluetoothManager.isBluetoothEnabled().then(()=> {
        ...
        return (
            <ScrollView style={styles.container}>
                ...
                <Button disabled={this.state.loading || !(this.state.bleOpend && this.state.boundAddress.length > 0 )}
                        title="ESC/POS" onPress={this.sendTextPrinter}/>
                </View>
            </ScrollView>
        );
    }
	...
	sendTextPrinter = (p) => {
		...
		BluetoothEscposPrinter.printText("Imprimiendo desde React 2 \r\n\r\n\r\n", { encoding:'GBK',     codepage:0, widthtimes:0, heigthtimes:1, fonttype:2});
		BluetoothEscposPrinter.printText("Imprimiendo desde React 3 \r\n\r\n\r\n", { encoding:'GBK',     codepage:0, widthtimes:0, heigthtimes:2, fonttype:3});
		BluetoothEscposPrinter.printText("Imprimiendo desde React 4 \r\n\r\n\r\n", { encoding:'GBK',        codepage:0, widthtimes:0, heigthtimes:3, fonttype:4});
		this.setState({
			loading: false
		});
    }
